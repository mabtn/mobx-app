...
export class OverlayService {
  ...
  open<P>(key: string, params: P, _options?: Record<string, unknown>): string {
    ...
    const session = def.createSession
      ? def.createSession(params, this.deps)
      : null;
    ...
    const instance: OverlayInstance<P> = {
      ...
      position: def.options?.defaultPosition ?? { x: 100 + this.modelessWindows.length * 30, y: 100 + this.modelessWindows.length * 30 },
      size: {
  ...
  private getList(kind: OverlayKind): OverlayInstance[] {
    switch (kind) {
      case "modal": return this.modalStack;
      case "modeless": return this.modelessWindows;
      case "popover": return this.popovers;
    }
  }
