...
function findLayer(deps: { document: { data: Record<string, any> } }, id: string): Layer | undefined {
...
const toggleVisibilityCmd: CommandDef<{ id: string }> = {
  ...
  toOps({ id }, deps) {
    ...
    return {
      ...
      inverseOps: [{ type: "editor:layer:setVisible", payload: { id, visible: layer.visible } }],
    };
  },
};
...
const setOpacityCmd: CommandDef<{ id: string; opacity: number }> = {
  ...
  toOps({ id, opacity }, deps) {
    ...
    return {
      ...
      inverseOps: [{ type: "editor:layer:setOpacity", payload: { id, opacity: layer.opacity } }],
    };
  },
  mergeKey({ id }) {
...
const setBlendModeCmd: CommandDef<{ id: string; blendMode: string }> = {
  ...
  toOps({ id, blendMode }, deps) {
    ...
    return {
      ...
      inverseOps: [{ type: "editor:layer:setBlendMode", payload: { id, blendMode: layer.blendMode } }],
    };
  },
};
...
const moveLayerCmd: CommandDef<{ id: string; x: number; y: number }> = {
  ...
  toOps({ id, x, y }, deps) {
    ...
    return {
      ...
      inverseOps: [{ type: "editor:layer:setPosition", payload: { id, x: layer.position.x, y: layer.position.y } }],
    };
  },
  mergeKey({ id }) {
...
const scaleLayerCmd: CommandDef<{ id: string; x: number; y: number }> = {
  ...
  toOps({ id, x, y }, deps) {
    ...
    return {
      ...
      inverseOps: [{ type: "editor:layer:setScale", payload: { id, x: layer.scale.x, y: layer.scale.y } }],
    };
  },
  mergeKey({ id }) {
...
const setBlurCmd: CommandDef<{ id: string; blur: number | undefined }> = {
  ...
  toOps({ id, blur }, deps) {
    ...
    return {
      ...
      inverseOps: [{ type: "editor:layer:setBlur", payload: { id, blur: layer.effects.blur } }],
    };
  },
  mergeKey({ id }) {
  ...
  toOps({ id, newOrder }, deps) {
    ...
    for (const l of layers) {
      ...
      if (shifted !== l.order) {
        ...
        inverseOps.push({ type: "editor:layer:setOrder", payload: { id: l.id, order: l.order } });
      }
    }
